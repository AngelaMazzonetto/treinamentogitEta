...
!
    ...
    & GSPREC
!
    NAMELIST /LOGICALNML/ SLOPE,GSPREC,LSST,LSSTMNTHLY,LCO2
!------------------------------------------------ 
...
!
    INTEGER(KIND=I4KIND)                                                                        ::&
    & I       , IER     , IERR    , ICLTEND, K      , CALC_NVEG,CALC_SST  , NREC_SST_UP ,         &
    & NOut
!
    CHARACTER(LEN=8)                                                                            ::&
    & ENVAR   , SRFILE
...
!----------------------------------------------- 
    ...
    110 FORMAT(I2)
        CLOSE(1)
!
        OPEN(UNIT=9, FILE="LOGICAL.nml", STATUS="OLD")
        READ(9,LOGICALNML)
        CLOSE(9)
!
        IF (MYPE == 0) THEN
...
!
            WRITE(6,*) 'MONTHLY TO DAY  SST UPDATE: ',LSSTMNTHLY
!
            IF (LSSTMNTHLY) THEN
...
!
        IF (SLOPE) THEN
           CALL HDIFFS
!GSM            CALL HDIFF  ! Test calling HDIFF for slope instead HDIFFS 
        ELSE
            CALL HDIFF
        END IF
!
